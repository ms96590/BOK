...
export class UIEventBus implements EventListenerObject {
  ...
  protected _hit_test_renderers(plot_view: PlotCanvasView, sx: number, sy: number): RendererView | null {
  ...
  protected _hit_test_frame(plot_view: PlotCanvasView, sx: number, sy: number): boolean {
    return plot_view.frame.bbox.contains(sx, sy)
  }

  protected _hit_test_canvas(plot_view: PlotCanvasView, sx: number, sy: number): boolean {
    return plot_view.layout.bbox.contains(sx, sy)
  }

  protected _hit_test_plot(sx: number, sy: number): PlotCanvasView | null {
    // TODO: z-index
    for (const plot_view of this.canvas_view.plot_views) {
      if (plot_view.layout.bbox.contains(sx, sy))
        return plot_view
    }
  ...
  protected _prev_move: {sx: number, sy: number, plot_view: PlotCanvasView | null} | null = null

  protected _curr_pan: {plot_view: PlotCanvasView} | null = null
  protected _curr_pinch: {plot_view: PlotCanvasView} | null = null
  protected _curr_rotate: {plot_view: PlotCanvasView} | null = null
  ...
  _trigger<E extends UIEvent>(signal: UISignal<E>, e: E, srcEvent: Event): void {
    ...
    if (e.type == "panstart" || e.type == "pan" || e.type == "panend") {
      let pan_view: PlotCanvasView | null
      ...
      let pinch_view: PlotCanvasView | null
      ...
      let rotate_view: PlotCanvasView | null
  ...
  __trigger<E extends UIEvent>(plot_view: PlotCanvasView, signal: UISignal<E>, e: E, srcEvent: Event): void {
